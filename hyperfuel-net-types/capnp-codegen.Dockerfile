...
WORKDIR /work
COPY hyperfuel_net_types.capnp .
RUN mkdir -p out \
    && capnp compile hyperfuel_net_types.capnp -o rust:out \
    && rustfmt out/hyperfuel_net_types_capnp.rs

# Minimal stage so `docker build --output` exports only the generated file.
FROM scratch AS export
COPY --from=codegen /work/out/hyperfuel_net_types_capnp.rs /
